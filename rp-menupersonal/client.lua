    ...
    ESX.UI.Menu.Open(
      'default', GetCurrentResourceName(), 'rp-menupersonal',
      {
        ...
        elements = {
          ...
          {label = _U('titulovehiculo'), value = 'titulovehiculo'},
          {label = _U('vehiculo'), value = 'interaccion_vehiculo'},
          {label = _U('personaje'), value = 'personaje'},
      ...
      function(data, menu)
        ...
        elseif data.current.value == 'interaccion_vehiculo' then
          ShowVehicleMenu()                                               
...
function ShowVehicleMenu()
  ...
  local hud = true
  local cinematica = false
  ESX.UI.Menu.Open('default',GetCurrentResourceName(),'interaccion_vehiculo',
  { 
    title = "Menu de Vehiculo", 
  align = 'bottom-right', 
  elements = {
    {label = _U('motor'), value = 'engine'},
    {label = _U('puertas'), value = 'puertas'},
    {label = _U('maletero'), value = 'maletero'},

  },
  }, function(data, menu)
    if data.current.value == 'engine' then
        ExecuteCommand("engine")                                                         -- Im using InfamesHUD
    elseif data.current.value == 'puertas' then
      ShowPuertasMenu() 
...
function ShowPuertasMenu()
  ...
  align = 'bottom-right', 
  elements = {
    {label = _U('puerta1'), value = 'puerta1'},
    {label = _U('puerta2'), value = 'puerta2'},
    {label = _U('puerta3'), value = 'puerta3'},
    {label = _U('puerta4'), value = 'puerta4'},

  },
  }, function(data, menu)
    if data.current.value == 'puerta1' then
        ExecuteCommand("puerta1")                                                         -- Im using InfamesHUD
    elseif data.current.value == 'puerta2' then
      ExecuteCommand("puerta2")
    elseif data.current.value == 'puerta3' then
      ExecuteCommand("puerta3")
    elseif data.current.value == 'puerta4' then
      ExecuteCommand("puerta4")
    end
  end, function(data, menu) 
  menu.close() 
  end)
